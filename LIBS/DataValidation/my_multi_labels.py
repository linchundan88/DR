...
def do_predict_dir(dicts_models, all_files, num_class=30, gpu_num=1):
    ...
    for i, model in enumerate(dicts_models):
        ...
        generator_test = my_images_generator_2d.My_images_generator_2d_test(all_files,
                                                                            batch_size=batch_size_test, image_shape=(image_size, image_size, 3))
...
def compute_probs(models, file_csv,
    ...
    for i, model in enumerate(models):
        ...
        generator_test = my_images_generator_2d.My_images_generator_2d_test(all_files,
                                                                            batch_size=batch_size, image_shape=(image_size, image_size, 3))
        for x in generator_test.gen():
            probabilities = model1.predict_on_batch(x)
