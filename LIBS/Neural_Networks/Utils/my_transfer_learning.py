...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, AveragePooling2D, GlobalAveragePooling3D, AveragePooling3D, Flatten
...
def add_top(model_input, num_output, dim='2D',activation_function='softmax'):
    ...
    output_layer = tensorflow.keras.layers.Dense(num_output, activation=activation_function)
    model1 = tensorflow.keras.Sequential([
        model_input,  global_average_layer,  output_layer
    ])
...
def convert_model_transfer(model_input, change_top=False, clsss_num=2,
    ...
    if isinstance(model_input, str):
        model1 = tensorflow.keras.models.load_model(model_input, compile=False)
    else:
        model1 = model_input
