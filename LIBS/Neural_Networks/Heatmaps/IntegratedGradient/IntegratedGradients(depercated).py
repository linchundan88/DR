...
class integrated_gradients:
    ...
    def __init__(self, model, outchannels=[], verbose=1):
        ...
        for c in self.outchannels:
            ...
            if K.backend() == "tensorflow":
                gradients = self.model.optimizer.__get_gradients(self.model.output[:, c], self.model.input)
            if K.backend() == "theano":
                gradients = self.model.optimizer.__get_gradients(self.model.output[:, c].sum(), self.model.input)
